...
Sequence:
  ...
  2. Transform guess — from [method::PrimeNumberTheorem](#methodprimenumbertheorem) (primes near x appear about every ln(x) numbers apart): a first guess p<sub>6</sub> &asymp; 6 &times; ln 6 &asymp; 10.75 — too small, the real 6th prime (13) is already past it
  3. Transform refine — the 6th prime is itself bigger than 6, so its own gap is a bit bigger than ln 6 too; use ln(6 &times; ln 6) = ln 6 + ln ln 6 &asymp; 2.375 in its place: 6 &times; (ln 6 + ln ln 6) &asymp; 14.25, rounded up to a ceiling of 15
...
Note: No encyclopedic source draws a picture for this inequality (checked: Wikipedia's Prime-counting function article states the bound as a formula only, no diagram; no published plot comparing p_n against the bound was found anywhere encyclopedic) — the idea itself is standard and sourced below; the picture is this catalog's own construction, honestly not attributed to an established visual tradition the way [method::SieveOfEratosthenes](#methodsieveoferatosthenes)'s grid is. Two earlier designs did not survive direct user feedback: a formula-in-a-box with no derivation at all ("doesn't answer how it's obtained, no geometry"), and 6 tiles laid end to end each just labeled with the formula itself ("useless" — restates the arithmetic, explains nothing about where the formula comes from). This version instead shows the standard heuristic derivation from [method::PrimeNumberTheorem](#methodprimenumbertheorem): a first, too-small guess (n&middot;ln n) demonstrably fails against the real answer, motivating the correction term ln ln n by name, not just by formula.
