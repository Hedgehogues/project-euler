...
Picture: — the honest drawing of this idea is two coprime pieces side by side, each with its own grid of divisors, the whole being every pairing of one from each; that is the SAME Cartesian grid [method::DivisorCountFormula](#methoddivisorcountformula) already owns, one level up, so drawing it here would be one picture serving twice (principle 11 allows a record at most one picture, and a second copy of a neighbour's is the duplication principle 9 forbids). No encyclopedic source draws the coprime-split property either — checked: Wikipedia's Multiplicative function, Divisor function and Coprime integers articles carry no figure for it, and cp-algorithms' divisors page is formulas and code only
Limits:
