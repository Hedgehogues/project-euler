...
## [method::EuclidsFormula]
Class: entity
Standard name: Euclid's formula (Pythagorean triple)
Essence: Every integer-sided right triangle is a whole-number multiple of one built from just two coprime integers of opposite parity — pick every valid pair, read its triangle straight off a formula, and credit every multiple of that triangle too, to reach every integer right triangle there is.
Recognized by: the statement is about right triangles with integer sides — Pythagorean triples — asking to enumerate them, count them by perimeter, or work with their sides, rather than testing one given triple for a²+b²=c²
General case: for integers m &gt; n &gt; 0 with gcd(m,n)=1 and m, n of opposite parity, `a = m&sup2;&minus;n&sup2;, b = 2mn, c = m&sup2;+n&sup2;` is a PRIMITIVE Pythagorean triple (its three sides share no common factor), and every primitive triple arises this way for exactly one such (m,n). Every OTHER integer right triangle is `k` times some primitive one for a positive integer k (`ka, kb, kc`), so enumerating every right triangle up to a limit means: walk every valid (m,n) pair whose primitive perimeter `2m(m+n)` does not exceed the limit, then credit that perimeter AND every multiple of it up to the limit — the same walk-the-multiples sweep as [method::SievingOverMultiples](#methodsievingovermultiples), here over the derived set of primitive perimeters rather than over the plain integers 1..N
Picture: ![Euclid's formula](visualizations/build/euclids-formula.png)
Sequence:
  1. Problem — m=2, n=1 given (coprime, opposite parity); what triangle do they build?
  2. Transform apply formula — each side read straight off m and n: a=2&sup2;&minus;1&sup2;=3, b=2&middot;2&middot;1=4, c=2&sup2;+1&sup2;=5
  3. Solution (k=1) — the primitive triple 3-4-5 (3&sup2;+4&sup2;=5&sup2;), perimeter 3+4+5=12
  4. Transform &times;k — scaling every side by k=2 gives 6-8-10, perimeter 24=2&times;12: every multiple of a primitive perimeter is a right-triangle perimeter too
Limits:
  - MUST: filter (m,n) by BOTH coprimality and opposite parity before treating the output as primitive — a limit of the IDEA: dropping either condition produces triples that are not primitive (already a multiple of a smaller one), and counting them as primitive double-credits the perimeters they share with that smaller triple
  - MUST NOT: stop at the primitive triple's own perimeter when the goal is every right triangle up to a limit — a limit of the IDEA: the non-primitive triples (Euclid's formula's own scaling fact) outnumber the primitive ones and are reached only by walking every multiple, not by running (m,n) further
  - MUST: the picture fixes m, n before applying the formula — a limit of the PICTURE: which (m,n) pairs satisfy the coprimality-and-parity filter is a separate enumeration question the formula itself does not answer
Source: [Wikipedia — Pythagorean triple](https://en.wikipedia.org/wiki/Pythagorean_triple) ("Euclid's formula ... generate[s] a Pythagorean triple ... The triple generated by Euclid's formula is primitive if and only if m and n are coprime and exactly one of them is even"; "every Pythagorean triple can be obtained ... by multiplying the elements of a primitive Pythagorean triple by a positive integer k"; the article's own worked example, m=2, n=1 giving (3,4,5), is what this picture's first three frames follow directly)
Example: `visualizations/examples/euclids-formula.{css,html}` (no js needed — the values are static) → `visualizations/build/euclids-formula.html`
Spec: [approaches](specs/approaches.md) · [visualizations](specs/visualizations.md)

## [method::ChampernowneConstant]
Class: entity
Standard name: Champernowne constant (the digit-locating use of its own block-offset function &delta;<sub>b</sub>(n))
Essence: Find the digit at any position of the decimal formed by concatenating every positive integer, WITHOUT ever building the string — subtract off, one digit-length block at a time, how many digits every shorter length already contributed, until the position lands inside the block of numbers having the current length; then divide to find which number in that block, and which of its own digits, holds the position.
Recognized by: the statement gives (or defines) the constant `0.123456789101112...` — every positive integer written out in order — and asks for the digit at some position, often for positions far too large to ever materialize the string itself
General case: for base 10, the number of digits contributed by every number of length shorter than `d` is &delta;<sub>10</sub>(d) = 9&middot;&sum;<sub>&#8467;=1</sub><sup>d-1</sup> 10<sup>&#8467;-1</sup>&middot;&#8467; (closed form: (1 + 10<sup>d-1</sup>((9)d&minus;10))/9); walking `d = 1, 2, 3, ...` and subtracting each block's own digit count (9&middot;10<sup>d-1</sup>&middot;d) from the target position `n` locates, in O(log n) steps, the length `d` whose block contains position `n`. The remaining offset then divides by `d`: the quotient selects WHICH `d`-digit number (counting from the first one, 10<sup>d-1</sup>), and the remainder selects which of that number's own `d` digits (leftmost first)
Picture: ![Champernowne digit locator](visualizations/build/champernowne-digit.png)
Sequence:
  1. Problem — the sequence 1 2 3 4 5 6 7 8 9 1 0 1 ? 1 3 ...; position n=12 is a `?`, and the d=1 block (9 one-digit numbers, 9 digits) is shown but not yet subtracted
  2. Transform subtract blocks — 12 &gt; 9, so the d=1 block is subtracted: 12&minus;9=3; since 3 fits inside the whole d=2 block (90&times;2=180 digits), the digit lands there
  3. Solution — which number: 10 + (3&minus;1)/2 = 11; which digit of it: (3&minus;1) mod 2 = 0 (leftmost); the leftmost digit of 11 is 1 — the digit at position 12
Limits:
  - MUST NOT: be confused with decomposing the digits of ONE already-known number — a limit of the IDEA: [method::PositionalNotation](#methodpositionalnotation) takes apart a single given number's own digits; this method first has to discover WHICH number and which of ITS digits even holds the asked-for position, across an unbounded concatenation of every number
  - MUST: subtract whole blocks before dividing within the last one — a limit of PRACTICE: dividing the raw position by a fixed digit-width without first locating the correct block width silently answers for the wrong length of number
