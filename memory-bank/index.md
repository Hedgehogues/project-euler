...
> Memory-bank хранит только переиспользуемое знание — приёмы и подходы, как учебник. Ни одной
> конкретной задачи и ни одной ссылки на задачу здесь нет — связь идёт в обратную сторону:
> задача (`euler{NNN}/README.md`) сама ссылается на нужный ей блок по имени. Задачи не
> индексируются здесь отдельно.
