...
criteria), which lives in [`memory-bank/specs/`](memory-bank/specs/) alongside the catalog's own
specs. Shared write-ups that don't belong to any single problem — the methods themselves, each with
its own picture — live in [`memory-bank/`](memory-bank/_terms.md).
